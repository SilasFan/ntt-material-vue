...
export default {
  ...
  computed: {
    ...
    time_now: function() {
      var today = new Date();
      return toString(today.getDate());
    },
    day_section: function() {
      var today = new Date();
      if (today.getHours() >= 6 && today.getHours() <= 12) return "早上";
      else if (today.getHours() > 12 && today.getHours() < 15) return "中午";
      else if (today.getHours() >= 15 && today.getHours() < 19) return "下午";
      else if (today.getHours() >= 19 && today.getHours() <= 24) return "晚上";
      else return "深夜";
    }
  }
};
